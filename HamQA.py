...
from modeling.modeling_hamqa import *
...
def main():
    ...
    parser.add_argument('--save_dir', default=f'./saved_models/hamqa/', help='model output directory')
    parser.add_argument('--save_model', default=True, dest='save_model', action='store_true')
    parser.add_argument('--load_model_path', default=None)
    parser.add_argument('--load_sentvecs_model_path', default=None)
    parser.add_argument('--without_amp', dest='without_amp', action='store_true',
    ...
    parser.add_argument('--counter_type', default=f'ham', help='graph soft counter or hard counter')
...
def train(args):
    ...
    dataset = LM_HamQA_DataLoader(args, args.train_statements, args.train_adj,
    ...
    #   Build model                                                                                   #
    ###################################################################################################
    model = LM_HamQA(args, args.encoder, k=args.k, n_ntype=4, n_etype=args.num_relation, enc_dim=args.enc_dim,
    ...
    for epoch_id in range(args.n_epochs):
            ...
            with open(preds_path, 'w') as f_preds:
                with torch.no_grad():
                    for qids, labels, *input_data in tqdm(eval_set):
                        ...
                        for i, (qid, label, pred, _preds_ranked) in enumerate(
                                zip(qids, labels, predictions, preds_ranked)):
        ...
        print('-' * 71)
        print('| epoch {:3} | step {:5} | dev_acc {:7.4f} | test_acc {:7.4f} |'.format(epoch_id, global_step, dev_acc,
                                                                                       test_acc))
        print('-' * 71)
        ...
        if dev_acc >= best_dev_acc:
            ...
            print('| epoch {:3} | step {:5} | best_dev_acc {:7.4f} | final_test_acc {:7.4f} |'.format(epoch_id,
                                                                                                      global_step,
                                                                                                      dev_acc,
                                                                                                      test_acc))
            # if args.save_model:
...
def eval_detail(args):
    ...
    model = LM_HamQA(old_args, old_args.encoder, k=old_args.k, n_ntype=4, n_etype=old_args.num_relation,
                     enc_dim=old_args.enc_dim,
                     fc_dim=old_args.fc_dim, n_fc_layer=old_args.fc_layer_num, p_fc=old_args.dropoutf,
